...
if __name__ == '__main__':
    ...
    for seed in seeds:
        for Model_Name in models:
            for nm in num_mali:
                args = args_parser()
                config_path = '/home/jingx/federated_learning_jx/federated_learning/GNN_common/configs/TUS/TUs_graph_classification_%s_%s_100k.json'%(Model_Name, args.dataset)
                ...
                for epoch in range(params['epochs']):
                    ...
                    save_path = os.path.join("/home/jingx/GS/Graph_Neural_Networks/federated_learning_jx/data/alpha/%d/CBA"%(seed), MODEL_NAME + '_' + args.dataset + \
                        '_%d_%d_%.2f_%.2f_%.2f'%(args.num_workers, nm, args.frac_of_avg, args.poisoning_intensity, args.density) + '_alpha.txt')
                    path = os.path.split(save_path)[0]
                    isExist = os.path.exists(path)
                    if not isExist:
                        os.makedirs(path)
