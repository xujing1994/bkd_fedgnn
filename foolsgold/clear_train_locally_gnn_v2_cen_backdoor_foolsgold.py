...
from Common.Utils.gnn_util import get_dataset, inject_global_trigger_test, transform_dataset, inject_global_trigger_train, load_pkl
import grpc
import copy
import time
from Common.Utils.evaluate import evaluate_accuracy, gnn_evaluate_accuracy_v2
    ...
    for seed in seeds:
        for nm in num_mali:
            ...
            filename = "/home/jingx/GS/Graph_Neural_Networks/federated_learning_jx/data/global_trigger_foolsgold/%d/%s_%s_%d_%d_%d_%.2f_%.2f_%.2f"\
                    %(args.seed, MODEL_NAME, config['dataset'], args.num_workers, nm, args.epoch_backdoor, args.frac_of_avg, args.poisoning_intensity, args.density) + '.pkl'
            global_trigger = load_pkl(filename)
            print("Triggers loaded!")
            nm = len(global_trigger)
            ...
            for epoch in range(params['epochs']):
                ...
                for i in range(args.num_workers):
                    ...
                    if not args.filename == "":
                        save_path = os.path.join(args.filename, '%d'%seed, 'CBA', config['model'] + '_' + args.dataset + '_%d_%d_%.2f_%.2f_%.2f'\
                                %(args.num_workers, nm, args.frac_of_avg, args.poisoning_intensity, args.density) + '_%d.txt'%i)
                ...
                save_path = os.path.join("/home/jingx/GS/Graph_Neural_Networks/federated_learning_jx/data/alpha/%d/CBA"%(seed), MODEL_NAME + '_' + args.dataset + \
                ...
                if not args.filename == "":
                    save_path = os.path.join(args.filename, '%d'%seed, 'CBA', MODEL_NAME + '_' + args.dataset + '_%d_%d_%.2f_%.2f_%.2f'\
