import sys
sys.path.append("/home/jingx/federated_learning_jx/federated_learning")
...
if __name__ == '__main__':
    seeds = range(10)
    num_mali = [2, 3]
    models = ['GCN', 'GAT', 'GraphSage', 'GatedGCN']
    ...
    for seed in seeds:
        for ModelName in models:
            for nm in num_mali:
                time_1 = time.time()
                args = args_parser()
                config_path = '/home/jingx/federated_learning_jx/federated_learning/GNN_common/configs/TUS/TUs_graph_classification_%s_%s_100k.json'%(ModelName, args.dataset)
                ...
                if nm > 0:
                    filename = "/home/jingx/GS/Graph_Neural_Networks/federated_learning_jx/data/global_trigger_foolsgold/%d/%s_%s_%d_%d_%d_%.2f_%.2f_%.2f"\
            %(seed, MODEL_NAME, config['dataset'], args.num_workers, nm, args.epoch_backdoor, args.frac_of_avg, args.poisoning_intensity, args.density) + '.pkl'
                ...
                for epoch in range(params['epochs']):
                    ...
                    for i in range(args.num_workers):
                        ...
                        if not args.filename == "":
                            save_path = os.path.join(args.filename, '%d'%seed, 'DBA', config['model'] + '_' + args.dataset + \
                                '_%d_%d_%.2f_%.2f_%.2f'%(args.num_workers, nm, args.frac_of_avg, args.poisoning_intensity, args.density) + '_%d.txt'%i)
                    ...
                    save_path = os.path.join("/home/jingx/GS/Graph_Neural_Networks/federated_learning_jx/data/alpha/%d/DBA"%(seed), MODEL_NAME + '_' + args.dataset + \
                        '_%d_%d_%.2f_%.2f_%.2f'%(args.num_workers, nm, args.frac_of_avg, args.poisoning_intensity, args.density) + '_global_test.txt')
                    path = os.path.split(save_path)[0]
                    isExist = os.path.exists(path)
                    if not isExist:
                        os.makedirs(path)
                    ...
                    if not args.filename == "":
                        save_path = os.path.join(args.filename,'%d'%seed, 'DBA', MODEL_NAME + '_' + args.dataset + '_%d_%d_%.2f_%.2f_%.2f'\
                                    %(args.num_workers, nm, args.frac_of_avg, args.poisoning_intensity, args.density) + '_global_test.txt')
                        path = os.path.split(save_path)[0]
                        isExist = os.path.exists(path)
                        if not isExist:
                            os.makedirs(path)
                    ...
                    if nm > 0 and epoch >= args.epoch_backdoor:
                        local_att_acc = []
                        ...
                        if not args.filename == "":
                            save_path = os.path.join(args.filename, '%d'%seed, 'DBA', MODEL_NAME + '_' + args.dataset + '_%d_%d_%.2f_%.2f_%.2f'\
                                    %(args.num_workers, nm, args.frac_of_avg, args.poisoning_intensity, args.density) + '_global_attack.txt')
                            path = os.path.split(save_path)[0]
                            isExist = os.path.exists(path)
                            if not isExist:
                                os.makedirs(path)
